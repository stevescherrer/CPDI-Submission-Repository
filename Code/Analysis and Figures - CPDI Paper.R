...
####### COMPONENT 1: Deep Water Range Test ----
  ...
  ### Using Dredge function to compare this GAM to all possible combinations of variables
      ...
      for(i in 1:length(candidate_models_1)){
        ...
        ## Saving summary stats to to model summary dataframe
         candidate_model_summary_1 = rbind(candidate_model_summary_1, cbind(candidate_model_id, paste(candidate_model_terms, sep = ' ', collapse = ' '), r.sq, dev.exp, candidate_model$aic, ave_max_distance, cpdi_extent))
      }
      colnames(candidate_model_summary_1) = c('model_id', 'model_terms', 'r.sq', 'dev.expl', 'aic', 'ave_max_distance', 'cpdi_extent')
    ...
    ## Average maximum detection radius
      amdr = fivenum(as.numeric(as.character(candidate_model_summary_1$ave_max_distance)))

    ## CPDI extent from candidate models
      cpdi_extent = fivenum(as.numeric(as.character(candidate_model_summary_1$cpdi_extent)))
  ...
  ### Running Mechanistic CPDI model for receiver position
       ...
       print(cpdi_prediction)
       ## Writing output of all this to table
       print(cbind(receiver_height, tag_height, tide_direction, time_of_day, paste(amdr, collapse =", "), paste(cpdi_extent, collapse =", "), cpdi_prediction))
       table_1 = rbind(table_1,  cbind(receiver_height, tag_height, tide_direction, time_of_day, paste(amdr, collapse =", "), paste(cpdi_extent, collapse =", "), cpdi_prediction))
       ...
       plot(x = subset_prediction_data$distance, y = subset_prediction_data$recovery_rate,  
            ylim=c(0,1), 
            main = 'GAM Criteron Estimation',
            xlab = 'Distance Between Tag and Receiver (m)', ylab = '% Transmissions Detected', 
            col = 'black',
            pch = 19,
            cex = .25
           )
       ...
       ## Adding CPDI to plot
       if(cpdi_extent[3] != 0){
         points(x = cpdi_extent[3], y = predicted_rates[cpdi_extent[3]], pch = 4)
      ...
      ## Saving table_1
      colnames(table_1) = c('Receiver Height (m)', 'Transmitter Height (m)', 'Tide Direction', 'Diurnal Period', 'Average Maximum Detection Radius (m)', 'CPDI Extent From GAM (m)', 'CPDI Prediction from Model (m)')
      setwd(results_dir)
      write.csv(table_1, 'CPDI_TABLE_1.csv')
...
###### COMPONENT 1: Shallow Water Range Test ----
  ...
  ### Fitting a GAM model 
    ...
    ### Using Dredge function to compare this GAM to all possible combinations of variables
      ## Exporting mod.gam and data to cluster for using pdredge function
        ...
        for(i in 1:length(candidate_models_2)){
          ...
          ## Determining difference between CPDI Extent and 0
          cpdi_extent - model_predictions$predicted_rates
    ...
    ### Average maximum detection radius
      amdr = fivenum(as.numeric(as.character(candidate_model_summary_2$ave_max_distance)))
        
    ### CPDI extent from candidate models
      cpdi_extent = fivenum(as.numeric(as.character(candidate_model_summary_2$cpdi_extent)))
    ...
    ### Running Mechanistic CPDI model for receiver position
      ...
      ## Writing output of all this to table
      table_2 = rbind(table_2, cbind(receiver_height, tide_direction, time_of_day, paste(amdr, collapse =", "), paste(cpdi_extent, collapse =", "), cpdi_prediction))
      # beep(1)
              }
          }
        }
        ...
        ## Saving table_2
        colnames(table_2) = c('Receiver Height (m)', 'Tide Direction', 'Diurnal Period', 'Average Maximum Detection Radius (m)', 'CPDI Extent From GAM (m)', 'CPDI Prediction from Model (m)')
        setwd(results_dir)
        write.csv(table_2, 'CPDI_TABLE_2.csv')
...
###### COMPONENT 3: Depth Dependent Validation Experiment ----
  ...
  #### Predicting CPDI / No CPDI for two different depth scenarios
   rec_50m =  predict_cpdi_interference(bottom_depth = 50,
                                ...
                                evaluation_interval = 1)
    plot_heat_map(rec_50m, save = TRUE, save_description = 'Component3_depth_dependent_50m.png')
   ...
   rec_215m =  predict_cpdi_interference(bottom_depth = 215,
                                ...
                                max_horizontal_dist = 50,
                                evaluation_interval = 1)
   plot_heat_map(rec_215m, save = TRUE, save_description = 'Component3_depth_dependent_215m.png')
   ...
   rec_212m =  predict_cpdi_interference(bottom_depth = 212,
                                           ...
                                           max_horizontal_dist = 50,
                                           evaluation_interval = 1)
   plot_heat_map(rec_212m, save = TRUE, save_description = 'Component3_depth_dependent_212m.png')
